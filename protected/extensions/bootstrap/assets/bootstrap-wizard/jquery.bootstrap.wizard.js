/*!
 * jQuery twitter bootstrap wizard plugin // moding!!
...
var bootstrapWizardCreate = function(element, options) {
	...
	this.fixNavigationButtons = function() {
		...
		$nextTab = $activeTab.find('a');

		if($settings.onTabShow && typeof $settings.onTabShow === 'function' && $settings.onTabShow($activeTab, $navigation, obj.currentIndex(),$nextTab)===false){
			return false;
		}
	};
	...
	this.next = function(e) {
		...
		$index = obj.nextIndex();
		var $nextTab = $navigation.find('li:eq('+$index+') a');

		if($settings.onNext && typeof $settings.onNext === 'function' && $settings.onNext($activeTab, $navigation, obj.nextIndex(),$nextTab)===false){
			return false;
		}
	...
	this.previous = function(e) {
		...
		if($settings.onPrevious && typeof $settings.onPrevious === 'function' && $settings.onPrevious($activeTab, $navigation, obj.previousIndex(),$nextTab)===false){
			return false;
		}
	...
	this.first = function(e) {
		if($settings.onFirst && typeof $settings.onFirst === 'function' && $settings.onFirst($activeTab, $navigation, obj.firstIndex(),$nextTab)===false){
			return false;
		}

		var $nextTab = $navigation.find('li:eq(0) a');
	...
	this.last = function(e) {
		if($settings.onLast && typeof $settings.onLast === 'function' && $settings.onLast($activeTab, $navigation, obj.lastIndex(),$nextTab)===false){
			return false;
		}
		var $nextTab = $navigation.find('li:eq('+obj.navigationLength()+') a');
		// If the element is disabled then we won't do anything
		if(element.hasClass('disabled')) {
			return false;
		}
